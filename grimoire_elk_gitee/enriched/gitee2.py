...
class GiteeEnrich2(Enrich):
    ...
    def get_rich_item(self, item):
        ...
        if item['category'] == 'issue':
            ...
            logger.error("[gitee] rich item not defined for gitee category {}".format(
                         item['category']))
    ...
    def get_rich_issue_comments(self, comments, eitem):
        ...
        for comment in comments:
            ...
            ecomment.pop('is_gitee2_{}'.format(ISSUE_COMMENT_TYPE))
    ...
    def get_rich_pull_reviews(self, comments, eitem):
        ...
        for comment in comments:
            ...
            # due to backtrack compatibility, `is_gitee2_*` is replaced with `is_gitee_*`
            ecomment.pop('is_gitee2_{}'.format(REVIEW_COMMENT_TYPE))
    ...
    def enrich_items(self, ocean_backend):
        ...
        if num_items != ins_items:
            ...
            logger.error("%s/%s missing items for gitee", str(missing), str(num_items))
        else:
            logger.info("%s items inserted for gitee", str(num_items))
    ...
    def __get_rich_pull(self, item):
        ...
        rich_pr["url_id"] = rich_pr['gitee_repo'] + "/pull/" + rich_pr['pull_id_in_repo']
        ...
        # due to backtrack compatibility, `is_gitee2_*` is replaced with `is_gitee_*`
        rich_pr.pop('is_gitee2_{}'.format(PULL_TYPE))
    ...
    def __get_rich_issue(self, item):
       ...
       #issue have four status: open,progressing, closed, rejected.
        ...
        rich_issue["url_id"] = rich_issue['gitee_repo'] + "/issues/" + rich_issue['issue_id_in_repo']
    ...
    def __get_rich_repo(self, item):
        ...
        # due to backtrack compatibility, `is_gitee2_*` is replaced with `is_gitee_*`
        rich_repo.pop('is_gitee2_{}'.format(REPOSITORY_TYPE))
        rich_repo['is_gitee_{}'.format(REPOSITORY_TYPE)] = 1
